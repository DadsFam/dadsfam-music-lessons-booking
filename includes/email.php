...
function mlb_send_email($to,$name,$instr,$date,$time,$dur,$price,$code,$extra=[]){
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:0 0 12px 12px;"><tr><td style="padding:24px 26px;">
  ...
  <p style="margin:0;font-size:13px;color:#64748b;">Save this code. Contact us if you need to cancel or reschedule.</p>
</td></tr>
<tr><td style="padding:0 26px 18px;text-align:center;"><p style="margin:0;font-size:11px;color:#94a3b8;">Automated message from '.esc_html($biz).'.</p></td></tr>
...
function mlb_send_cancellation_email( string $to, string $name, string $instr, string $date, string $time, string $code ): void {
    $sym = mlb_sym();
    $biz = mlb_setting( 'business_name', get_bloginfo('name') );
...
<tr><td style="padding:0 26px 18px;text-align:center;">
  <p style="margin:0;font-size:11px;color:#94a3b8;">Automated message from ' . esc_html( $biz ) . ' — please do not reply.</p>
</td></tr></table>
</td></tr></table>
</body></html>';
